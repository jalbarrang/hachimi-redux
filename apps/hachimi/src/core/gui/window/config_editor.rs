...
impl ConfigEditor {
    ...
    /// opacity + per-panel show/hide and reset-position. Runtime overlay prefs
    /// (apply immediately, not part of the Save/Cancel working copy). The global
    /// lock toggle was dropped per design. (Per-overlay *hotkey* toggles are the
