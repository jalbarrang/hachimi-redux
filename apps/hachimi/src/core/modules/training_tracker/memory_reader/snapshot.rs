...
use crate::core::modules::training_tracker::evaluation::Aptitudes;
...
pub struct CareerSnapshot {
    ...
    /// Mapped to a rank-badge label via `crate::core::modules::training_tracker::rank_table::rank_label`.
    ...
    /// `crate::core::modules::training_tracker::chara_effects`. Empty when none are active or unread.
...
fn read_snapshot_inner() -> Option<CareerSnapshot> {
    ...
    let chara_effect_ids = unsafe { read_obscured_int_array(effect_array) };
    {
        ...
        // to display names (see crate::core::modules::training_tracker::chara_effects).
        ...
        if !chara_effect_ids.is_empty() && !EFFECTS_LOGGED.swap(true, Ordering::Relaxed) {
            ...
            let unknown: Vec<i32> = chara_effect_ids
                .iter()
                .copied()
                .filter(|&id| !crate::core::modules::training_tracker::chara_effects::is_known(id))
                .collect();
            if !unknown.is_empty() {
    ...
    Some(CareerSnapshot {
        ...
        aptitudes,
        star,
        // Filled by overlay_cache (self-computed via crate::core::modules::training_tracker::evaluation).
