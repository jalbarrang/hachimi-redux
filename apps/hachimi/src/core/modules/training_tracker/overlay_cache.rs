...
use crate::core::modules::training_tracker::deck_bonuses;
...
fn refresh_cache_inner() {
    ...
    if is_playing {
        crate::core::modules::training_tracker::bond_progress::observe(&support_ids, &fired_events);
    } else {
        crate::core::modules::training_tracker::bond_progress::clear();
    }
    if is_playing {
        ...
        if let Some(s) = snapshot.as_mut() {
            ...
            s.evaluation_value =
                crate::core::modules::training_tracker::evaluation::compute(stats, &s.aptitudes, s.star, &skills);
        }
    ...
    crate::core::modules::training_tracker::telemetry::publish(
        snap_for_pub.as_ref(),
        &skills,
        &evaluations,
        &skill_shop,
...
fn log_career_diagnostic(evaluations: &[EvaluationInfo], support_ids: &[(i32, i32)], fired: &[FiredEvent]) {
    ...
    for (slot, support_id) in support_ids {
        let name =
            crate::core::modules::training_tracker::gametora_data::support_card_name(*support_id as i64).unwrap_or("?");
        let max = crate::core::modules::training_tracker::gametora_data::max_chain_steps(*support_id as i64);
        let keys = crate::core::modules::training_tracker::gametora_data::chain_event_keys(*support_id as i64);
        let matched = keys
            .iter()
            .filter(|(eid, sid)| {
