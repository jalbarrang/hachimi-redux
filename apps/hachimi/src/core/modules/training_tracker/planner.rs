...
//! per-turn scorer in [`crate::core::modules::training_tracker::recommend`] by layering three trajectory-aware
...
//!    distance-from-target gap ([`crate::core::modules::training_tracker::build_profile`]) are boosted, shifting the
...
use crate::core::modules::training_tracker::recommend::{self, FacilityScore, RecommendParams, TurnSuggestion};
...
/// [`crate::core::modules::training_tracker::config`]); every field is `#[serde(default)]` for clean migration.
...
/// Replace the parameters (clamped). Call [`crate::core::modules::training_tracker::config::persist`] to save.
