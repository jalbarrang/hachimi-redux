...
//! keyed by its basename (see [`crate::core::modules::training_tracker::race_context`]).

use std::collections::HashMap;
use std::sync::{Mutex, OnceLock};

use crate::core::modules::training_tracker::compat::egui::{self, Color32, ColorImage, TextureHandle, TextureOptions};

/// `(basename, png-bytes)` for every embedded icon. The basenames match
/// `*::icon_name()` in [`crate::core::modules::training_tracker::race_context`].
const ASSETS: &[(&str, &[u8])] = &[
    (
        "utx_ico_weather_00",
