        ...
        match get_version() {
            ...
            Err(_) => {
                unsafe {
                    MessageBoxW(
                        None,
                        w!("Unable to get Hachimi version from GitHub. Are you online?"),
                        w!("Error"),
                        MB_ICONERROR | MB_OK,
                    )
                };
        ...
        WM_COMMAND => {
            ...
            match control_id {
                ...
                IDC_INSTALL_PATH_BROWSE => {
                    let installer = get_installer(dialog);
                    let Some(path) =
                        utils::open_select_folder_dialog(dialog, installer.install_dir.as_ref().filter(|p| p.is_dir()))
                    else {
                        return 1;
                    };
                ...
                IDC_UNINSTALL => {
                    ...
                    if res == IDOK {
                        ...
                        // Remove the Training Tracker plugin alongside the core mod.
                        _ = installer.uninstall_training_tracker();
                        update_target(dialog, GetDlgItem(dialog, IDC_TARGET).unwrap(), installer.target as _);
