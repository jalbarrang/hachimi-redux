...
pub fn render_menu(ui: &mut hachimi_plugin_sdk::egui::Ui) {
    ...
    MENU_MOUNT.with(|slot| render_slot(slot, menu_shell, ui));
}

pub fn render_overlay(ui: &mut hachimi_plugin_sdk::egui::Ui) {
    set_native_draw(super::draw_overlay_inner);
    OVERLAY_MOUNT.with(|slot| render_slot(slot, overlay_shell, ui));
}

fn render_slot(slot: &RefCell<Option<UiMount>>, app: fn() -> Element, ui: &mut hachimi_plugin_sdk::egui::Ui) {
