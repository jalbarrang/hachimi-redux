...
Em caso de dúvida, recompile o plugin contra este repositório (host API v9) em vez de depender do caminho legado.
...
O HachimiRedux é o mod principal (carregado como `cri_mana_vpx.dll`); o **Training Tracker** é um plugin DLL opcional carregado pelo mod principal. Ambos são compilados a partir deste repositório e têm de vir do mesmo build.
